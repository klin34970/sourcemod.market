...
@section('page', 'page-jobs-discussions') 
@section('content')
<!-- START @PAGE CONTENT -->
						...
						<div class="tab-content">
							<div class="tab-pane fade in active" id="tab2">
								<div class="panel-body no-padding">
									@foreach($job->discussions()->paginate((int)config('sourcemod.market.discussions_pagination')) as $discussion)
                                    <div class="_post media inner-all no-margin" data-author-name="{{ $discussion->user_profile->first()->first_name }}" data-quote-body="{!! htmlentities($discussion->text) !!}">
										...
										@if(isset($logged_user))
											...
											@if(
												...
												<a href="{!! URL::route('discussions.jobs.delete', ['id' => $discussion->id, '_token' => csrf_token()]) !!}"><i class="fa fa-close"></i></a>
                                        ...
                                        <div class="media-body">
                                            ...
                                            <em class="text-xs text-muted">{{ trans('front/jobs/view.posted') }} <span class="text-danger">{{ $discussion->created_at->diffForHumans() }}</span></em>
                                        </div><!-- /.media-body -->
                                    </div><!-- /.media -->
								...
								<div class="text-center">
									{{$job->discussions()->paginate((int)config('sourcemod.market.discussions_pagination'))->links()}}
								</div>
								...
								<div class="panel rounded shadow">
									<div class="panel-body no-padding">
										<div class="form-body">
											{!! Form::model($job, [ 'url' => URL::route('jobs.discussions.add', ['id' => $job->id, 'game' => App\Http\Classes\Slug::filter($job->game->title), 'category' => App\Http\Classes\Slug::filter($job->category), 'title' => App\Http\Classes\Slug::filter($job->title) ]), 'class' => 'form-horizontal mt-10'] )  !!}
